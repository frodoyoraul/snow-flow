...
logger = logging.getLogger('snow_order_onboarding_v3')
...
# Mapeo de códigos de tipo en ServiceNow (item_option_new.type)
# Basado en valores comunes; ajustar según se necesite
TYPE_MAP = {
    1: "String",
    2: "Text",  # Multiline
    6: "Single Line Text",
...
def normalize_type(dtype):
    """Convierte código numérico o string a nombre de tipo."""
    if dtype is None:
        return None
    # Si es dict, extraer valor
    if isinstance(dtype, dict):
        dtype = dtype.get("display_value") or dtype.get("value")
    # Si ya es string, devolverlo tal cual (ya es legible)
    if isinstance(dtype, str):
        return dtype
    # Si es número (int/float), mapear usando TYPE_MAP
    try:
        # Convertir a entero
        if isinstance(dtype, (int, float)):
            num = int(dtype)
            return TYPE_MAP.get(num, str(dtype))
...
def build_variables_dict_by_sysid(variables_records, abraham_id, location_id, department_id, manager_id):
    ...
    for var in variables_records:
        ...
        if mandatory:
            # Reference (puede venir como "Reference" o como número 8)
            if (dtype == "Reference" or dtype == 8) and ref_table:
            ...
            # Email (26 o "Email")
                ...
                if "personal" in lbl_low:
                    ...
                    value = ABRAHAM_EMAIL
            # Date/Time (10 o "Date/Time")
            elif dtype == "Date/Time" or dtype == "Date" or dtype == 10:
                value = START_DATE
            # Texto (String, Single Line Text, Text, o códigos 1,2,6)
            elif dtype in ["String", "Single Line Text", "Text"] or dtype in [1, 2, 6]:
                if "first name" in lbl_low:
                    ...
                    value = ""  # Otros obligatorios sin ejemplo
            else:
...
async def main():
    logger.info("=== Iniciando pedido de User Onboarding (v3: mapeo de tipos) ===")
