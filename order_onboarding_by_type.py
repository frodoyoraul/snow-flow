...
logger = logging.getLogger('snow_order_onboarding_v4')
...
def build_variables_dict_by_sysid(variables_records, abraham_id, location_id, department_id, manager_id):
    """
    Asigna valores a las variables basándose en tipo/referencia, sin depender de etiquetas.
    """
    var_dict = {}
    
    # Contadores para tipos de texto
    text_counter = 0
    ...
    for var in variables_records:
        ...
        # Normalizar tipo a entero si es posible
        dtype = None
        if isinstance(dtype_raw, (int, float)):
            dtype = int(dtype_raw)
        ...
        # Solo nos interesan las mandatory (las no mandatory pueden dejarse vacías)
        if not mandatory:
            var_dict[var_sys_id] = ""
            continue
        
        # Asignar valor según tipo y referencia
        value = ""
        
        # Reference (tipo 8)
        if dtype == 8 and ref_table:
            if ref_table == "sys_user":
                # Asumimos que es la variable Manager (única referencia a sys_user en onboarding)
                value = manager_id
                ...
                logger.warning("Referencia no manejada: %s", ref_table)
        # Email (tipo 26)
        elif dtype == 26:
            value = ABRAHAM_EMAIL
        # Date/Time (tipo 10)
            ...
            value = START_DATE
        # Single Line Text / String (tipo 6 o 1 o 2)
        elif dtype in [6, 1, 2]:
            # Asignamos según orden de aparición: first, last, job title
            if text_counter == 0:
                ...
                value = ""  # Por si hay más textos
            text_counter += 1
        else:
            logger.warning("Tipo no manejado para variable mandatory: %s", dtype)
        
        var_dict[var_sys_id] = value
        logger.debug("Variable %s (type %s, ref %s): %s", var_sys_id, dtype, ref_table, value)
...
async def main():
    logger.info("=== Iniciando pedido de User Onboarding (v4: mapeo por tipo sin etiquetas) ===")
